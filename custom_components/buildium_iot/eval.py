"""Python interpreter for buildium_iot."""
        ...
        if eval_globals is None and eval_ast.sym_table != ast_ctx.sym_table:
            for var, value in eval_ast.sym_table.items():
                ...
                if var not in ast_ctx.sym_table and (
                    ast_ctx.curr_func is None
                    or var not in ast_ctx.curr_func.local_names
                ):
...
class EvalFunc:
    """Class for a callable buildium_iot function."""
    ...
    async def trigger_init(self):
        ...
        for dec in self.decorators:
                ...
                for arg_num, arg in enumerate(dec_args):
                    ...
                    if "type" in arg_info:
                        if type(arg) in arg_info["type"]:
                            ...
                            mesg += ", or " + ", or ".join(
                                sorted(
                                    [ok_type.__name__ for ok_type in arg_info["type"]]
                                )
                            )
                    raise TypeError(
            ...
            if dec_name == "service":
                ...
                if desc is None or desc == "":
                    desc = f"buildium_iot function {self.name}()"
                desc = desc.lstrip(" \n\r")
                ...
                def pyscript_service_factory(func_name, func):
                    async def pyscript_service_handler(call):
                        ...
                        #
                        ast_ctx = AstEval(
                            f"{self.global_ctx_name}.{func_name}", self.global_ctx
                        )
        ...
        while True:
            ...
            self.trigger.append(
                self.global_ctx.get_trig_info(
                    f"{self.global_ctx_name}.{self.name}", trig_args
                )
            )
    ...
    async def resolve_nonlocals(self, ast_ctx):
        ...
        for var_name in var_names:
            ...
            for sym_table in reversed(
                ast_ctx.sym_table_stack[sym_table_idx:] + [ast_ctx.sym_table]
            ):
...
class EvalFuncVarClassInst(EvalFuncVar):
    """Class for a callable buildium_iot class instance function."""
    ...
    async def __call__(self, *args, **kwargs):
        ...
        return await self.func.call(
            self.ast_ctx, self.class_inst_weak(), *args, **kwargs
        )
...
class AstEval:
    ...
    async def ast_functiondef(self, arg):
        ...
        if pyscript_compile:
            ...
            local_var = None
            if arg.name in self.sym_table and isinstance(
                self.sym_table[arg.name], EvalLocalVar
            ):
            ...
            exec(
                code, self.global_sym_table, self.sym_table
    ...
    async def recurse_assign(self, lhs, val):
        ...
        if isinstance(lhs, ast.Tuple):
            ...
            if self.curr_func and var_name in self.curr_func.global_names:
                ...
                return
            if var_name in self.sym_table and isinstance(
                self.sym_table[var_name], EvalLocalVar
            ):
    ...
    async def loopvar_scope_save(self, generators):
        ...
        return lvars, {
            var: self.sym_table[var] for var in lvars if var in self.sym_table
        }
        ...
        if inspect.isclass(func) and hasattr(func, "__init__evalfunc_wrap__"):
            ...
            for name in inst.__dir__():
                ...
                setattr(
                    inst,
                    name,
                    EvalFuncVarClassInst(
                        value.get_func(), value.get_ast_ctx(), inst_weak
                    ),
                )
    ...
    async def get_names(
        self, this_ast=None, nonlocal_names=None, global_names=None, local_names=None
    ):
        ...
        try:
            ...
            if err.filename == self.filename:
                ...
                self.exception_long = self.format_exc(
                    err, 1, self.col_offset, code_list=[err.text]
                )
            return False
        except asyncio.CancelledError:
            raise
