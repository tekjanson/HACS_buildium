...
class TrigTime:
    ...
    def init(cls, hass):
        ...
        def user_task_create_factory(ast_ctx):
            ...
            async def user_task_create(func, *args, **kwargs):
                ...
                new_ast_ctx = AstEval(
                    f"{ast_ctx.get_global_ctx_name()}.{func_name}",
    ...
    async def user_task_executor(cls, func, *args, **kwargs):
        ...
        if isinstance(func, EvalFuncVar):
            raise TypeError(
                "buildium_iot functions can't be called from task.executor - must be a regular python function"
            )
        ...
        for spec in time_spec:
            ...
            if cron_match:
                ...
                if this_t <= now and this_t != startup_time:
                    ...
                    #
                    this_t = cls.parse_date_time(
                        match1[1].strip(), 1, now, startup_time
                    )
                ...
                if period <= 0:
                    _LOGGER.error(
                        "Invalid non-positive period %s in period(): %s",
                        period,
                        time_spec,
                    )
                    continue
...
class TrigInfo:
    ...
    async def trigger_watch(self):
        ...
        try:
            ...
            if self.event_trigger is not None:
                _LOGGER.debug(
                    "trigger %s adding event_trigger %s",
                    self.name,
                    self.event_trigger[0],
                )
                Event.notify_add(self.event_trigger[0], self.notify_q)
            if self.mqtt_trigger is not None:
                _LOGGER.debug(
                    "trigger %s adding mqtt_trigger %s", self.name, self.mqtt_trigger[0]
                )
            ...
            check_state_expr_on_start = (
                self.state_check_now or self.state_hold_false is not None
            )
            ...
            while True:
                ...
                if self.run_on_startup:
                    ...
                    if self.time_trigger:
                        time_next = TrigTime.timer_trigger_next(
                ...
                if state_trig_timeout:
                    ...
                    if not ident_any_values_changed(
                        func_args, self.state_trig_ident_any
                    ):
                        #
                ...
                if trig_ok and self.active_expr:
                    active_vars = State.notify_var_get(
                        self.state_active_ident, new_vars
                    )
                ...
                if trig_ok and self.time_active:
                    trig_ok = TrigTime.timer_active_check(
                        self.time_active, now, startup_time
                    )
    ...
    def call_action(self, notify_type, func_args, run_task=True):
        ...
        async def do_func_call(
            func, ast_ctx, task_unique, task_unique_func, hass_context, **kwargs
        ):
