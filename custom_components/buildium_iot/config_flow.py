...
CONF_STR_ALL = {CONF_BUILDIUM_CLIENT_ID, CONF_BUILDIUM_SECRET}
CONF_ALL = {
    CONF_ALLOW_ALL_IMPORTS,
    CONF_HASS_IS_GLOBAL,
    CONF_BUILDIUM_CLIENT_ID,
...
PYSCRIPT_SCHEMA = vol.Schema(
    {
        ...
        vol.Optional(
            CONF_BUILDIUM_CLIENT_ID, default="1ef18add-629d-4f93-8121-a0d5f4fa7a1b"
        ): str,
        vol.Optional(
            CONF_BUILDIUM_SECRET, default="zfe1szl8sbYH1z6YybbqFEYLPaJ0BtS8x6R9i3eUmNY="
        ): str,
    },
...
class PyscriptOptionsConfigFlow(config_entries.OptionsFlow):
    """Handle a buildium_iot options flow."""

    def __init__(self, config_entry: ConfigEntry) -> None:
        """Initialize buildium_iot options flow."""
    ...
    async def async_step_init(
        ...
        """Manage the buildium_iot options."""
        ...
        if user_input is None:
            return self.async_show_form(
                ...
                #     {
                #         vol.Optional(
                #             name, default=self.config_entry.data.get(name, False)
                #         ): bool
...
class PyscriptConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    """Handle a buildium_iot config flow."""
    ...
    async def async_step_import(
        ...
        if entries:
            ...
            for key, val in import_config.items():
                if entry.source == SOURCE_IMPORT or key not in CONF_ALL:
            ...
            for key in entry.data:
                if (
                    (entry.source == SOURCE_IMPORT or key not in CONF_ALL)
