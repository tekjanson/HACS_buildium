"""Requirements helpers for buildium_iot."""
import glob
import logging
import os
import sys
...
def process_all_requirements(pyscript_folder, requirements_paths, requirements_file):
    ...
    for root in requirements_paths:
        for requirements_path in glob.glob(
            os.path.join(pyscript_folder, root, requirements_file)
        ):
        ...
        for pkg in pkg_lines:
            ...
            try:
                ...
                # pinned version
                    ...
                    new_version == UNPINNED_VERSION
                    and current_pinned_version != UNPINNED_VERSION
                ):
                    _LOGGER.warning(
                        (
                ...
                # path so we can show sources
                    ...
                    all_requirements_to_install[pkg_name][ATTR_SOURCES].append(
                        requirements_path
                    )
...
async def install_requirements(hass, config_entry, pyscript_folder):
    ...
    pyscript_installed_packages = config_entry.data.get(
        CONF_INSTALLED_PACKAGES, {}
    ).copy()
            ...
            if package in pyscript_installed_packages and Version(
                ...
                _LOGGER.warning(
                    (
                        ...
                        " the version '%s' which was installed outside of buildium_iot"
                    ),
                    version_to_install,
                    package,
                    str(sources),
    ...
    pyscript_installed_packages.update(
        {
            package: requirements_to_install[package][ATTR_VERSION]
            for package in requirements_to_install
        }
    )
    ...
    if any(
        version == UNPINNED_VERSION for version in pyscript_installed_packages.values()
    ):
        pyscript_installed_packages = await hass.async_add_executor_job(
            update_unpinned_versions, pyscript_installed_packages
        )
    if pyscript_installed_packages != config_entry.data.get(
        CONF_INSTALLED_PACKAGES, {}
    ):
