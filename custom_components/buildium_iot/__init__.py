...
from .const import (
    ...
    CONF_HASS_IS_GLOBAL,
    CONF_BUILDIUM_CLIENT_ID,
    CONF_BUILDIUM_SECRET,
    CONFIG_ENTRY,
    CONFIG_ENTRY_OLD,
    DOMAIN,
    FOLDER,
...
PYSCRIPT_SCHEMA = vol.Schema(
    {
        ...
        vol.Optional(CONF_BUILDIUM_CLIENT_ID, default="empty"): cv.string,
        vol.Optional(CONF_BUILDIUM_SECRET, default="empty"): cv.string,
    },
...
def start_global_contexts(global_ctx_only: str = None) -> None:
    ...
    for global_ctx_name, global_ctx in GlobalContextMgr.items():
        ...
        if global_ctx_only is not None and global_ctx_only != "*":
            if global_ctx_name != global_ctx_only and not global_ctx_name.startswith(
                global_ctx_only + "."
            ):
                continue
...
async def watchdog_start(
    hass: HomeAssistant,
    pyscript_folder: str,
    reload_scripts_handler: Callable[[None], None],
) -> None:
    ...
    async def task_watchdog(watchdog_q: asyncio.Queue) -> None:
        ...
        while True:
            try:
                ...
                _LOGGER.error(
                    "task_watchdog: got exception %s", traceback.format_exc(-1)
                )
    ...
    if observer is not None:
        ...
        hass.data[DOMAIN][WATCHDOG_TASK] = Function.create_task(
            task_watchdog(watchdog_q)
        )
...
async def async_setup_entry(hass: HomeAssistant, config_entry: ConfigEntry) -> bool:
    ...
    scope_cheat_id = config_entry.data.get(CONF_BUILDIUM_CLIENT_ID, False)
    scope_cheat_secret = config_entry.data.get(CONF_BUILDIUM_SECRET, False)
    pyscript_folder = hass.config.path(FOLDER)
    src_example = os.getcwd() + "/custom_components/buildium_iot/example.py"
    src_property_unit = (
        os.getcwd() + "/custom_components/buildium_iot/property_unit_pull.py"
    )
    src_blueprint = os.getcwd() + "/custom_components/buildium_iot/blueprint.yaml"
    target_blueprint = (
        "/config/blueprints/automation/homeassistant/buildium_iot_blueprint.yaml"
    )
    ...
    if not await hass.async_add_executor_job(os.path.isdir, pyscript_folder):
        ...
        _LOGGER.info("Folder %s not found in configuration folder, creating it", FOLDER)
        await hass.async_add_executor_job(os.makedirs, pyscript_folder)
        _LOGGER.info("trying to find %s", pyscript_example)

        _LOGGER.info("trying to put at %s", src_example)
        await hass.async_add_executor_job(copyfile, src_example, pyscript_example)
        await hass.async_add_executor_job(
            copyfile, src_property_unit, pyscript_property_unit
        )
        await hass.async_add_executor_job(copyfile, src_blueprint, target_blueprint)

    _LOGGER.info("Unloading all scripts %s", config_entry)
    # f = open("creds.json", "w")
    # f.write(str({}))
    hass.data.setdefault(DOMAIN, {})
    ...
    _LOGGER.info(
        f"asdfasdfasdfasdf{hass.services.has_service('buildium_iot', 'reload')}"
    )
    # await hass.services.async_call(
    #     "buildium_iot",
    #     "reload",
    ...
    # scope_cheat_id=""
    # scope_cheat_secret=""
    # buildium_iot.property_unit_pull
    await hass.services.async_call(
        "buildium_iot",
        "property_unit_pull",
        {
            "buildium_client_id": scope_cheat_id,
            "buildium_client_secret": scope_cheat_secret,
        },
    )
    ...
    async def jupyter_kernel_start(call: ServiceCall) -> None:
        ...
        global_ctx = GlobalContext(
            global_ctx_name,
            global_sym_table={"__name__": global_ctx_name},
    ...
    hass.data[DOMAIN][UNSUB_LISTENERS].append(
        hass.bus.async_listen(EVENT_HOMEASSISTANT_STOP, hass_stop)
    )
...
async def load_scripts(
    hass: HomeAssistant, config_data: Dict[str, Any], global_ctx_only: str = None
):
        ...
        for path, match, check_config, autoload in load_paths:
            for this_path in sorted(
                ...
                try:
                    ...
                    _LOGGER.error(
                        "load_scripts: skipping %s due to exception %s", this_path, exc
                    )
                    continue
    ...
    ctx2files = await hass.async_add_executor_job(
        glob_read_files, load_paths, apps_config
    )
    ...
    if global_ctx_only is not None and global_ctx_only != "*":
        if global_ctx_only not in ctx_all and global_ctx_only not in ctx2files:
            _LOGGER.error(
                "buildium_iot.reload: no global context '%s' to reload", global_ctx_only
            )
            return
    ...
    if len(will_reload) > 0:
        ...
        def import_recurse(ctx_name, visited, ctx2imports):
            ...
            for imp_name in ctx.get_imports():
                ...
                ctx2imports[ctx_name].update(
                    import_recurse(imp_name, visited, ctx2imports)
                )
    ...
    for global_ctx_name, src_info in ctx2files.items():
        if not src_info.force:
            continue
        if not global_ctx_name.startswith("apps.") and not global_ctx_name.startswith(
            "modules."
        ):
            continue
        parts = global_ctx_name.split(".")
