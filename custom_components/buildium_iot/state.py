...
class State:
    ...
    #
    # buildium_iot yaml configuration
    #
    pyscript_config = {}

    #
    # buildium_iot vars which have already been registered as persisted
    #
    persisted_vars = set()
    ...
    async def update(cls, new_vars, func_args):
        ...
        if notify:
            ...
            for queue, var_names in notify.items():
                await queue.put(
                    ["state", [cls.notify_var_get(var_names, new_vars), func_args]]
                )
    ...
    async def register_persist(cls, var_name):
        """Register buildium_iot state variable to be persisted with RestoreState."""
        if var_name.startswith("buildium_iot.") and var_name not in cls.persisted_vars:
    ...
    async def persist(cls, var_name, default_value=None, default_attributes=None):
        """Persist a buildium_iot domain state variable, and update with optional defaults."""
        if var_name.count(".") != 1 or not var_name.startswith("buildium_iot."):
            raise NameError(
                f"invalid name {var_name} (should be 'buildium_iot.entity')"
            )
                ...
                async def service_call(*args, **kwargs):
                    ...
                    for keyword, typ, default in [
                        (
                            "context",
                            [Context],
                            Function.task2context.get(curr_task, None),
                        ),
                        ("blocking", [bool], None),
                    ...
                    await cls.hass.services.async_call(
                        domain, service, kwargs, **hass_args
                    )
    ...
    def register_functions(cls):
        ...
        functions = {
            ...
            "buildium_iot.config": cls.pyscript_config,
        }
        Function.register(functions)

    @classmethod
    def set_pyscript_config(cls, config):
        """Set buildium_iot yaml config."""
        #
