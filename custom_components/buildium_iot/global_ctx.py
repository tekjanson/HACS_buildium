...
class GlobalContext:
    ...
    def __init__(
        ...
        self.name: str = name
        self.global_sym_table: Dict[str, Any] = (
            global_sym_table if global_sym_table else {}
        )
        ...
        config_entry: ConfigEntry = Function.hass.data.get(DOMAIN, {}).get(
            CONFIG_ENTRY, {}
        )
        ...
        if app_config:
            self.global_sym_table["buildium_iot.app_config"] = app_config.copy()
    ...
    async def module_import(
        ...
        """Import a buildium_iot module from the buildium_iot/modules or apps folder."""
        ...
        def find_first_file(
            file_paths: List[Set[str]],
        ) -> List[Optional[Union[str, ModuleType]]]:
        ...
        if import_level > 0:
            ...
            if self.rel_import_path is not None and self.rel_import_path.startswith(
                "apps/"
            ):
        ...
        #
        file_info = await Function.hass.async_add_executor_job(
            find_first_file, file_paths
        )
        if not file_info:
            return [None, None]
...
class GlobalContextMgr:
    ...
    def init(cls) -> None:
        ...
        def get_global_ctx_factory(ast_ctx: AstEval) -> Callable[[], str]:
            """Generate a buildium_iot.get_global_ctx() function with given ast_ctx."""
        ...
        def list_global_ctx_factory(ast_ctx: AstEval) -> Callable[[], List[str]]:
            """Generate a buildium_iot.list_global_ctx() function with given ast_ctx."""
        ...
        def set_global_ctx_factory(ast_ctx: AstEval) -> Callable[[str], None]:
            """Generate a buildium_iot.set_global_ctx() function with given ast_ctx."""
        ...
        ast_funcs = {
            "buildium_iot.get_global_ctx": get_global_ctx_factory,
            "buildium_iot.list_global_ctx": list_global_ctx_factory,
            "buildium_iot.set_global_ctx": set_global_ctx_factory,
        }
    ...
    async def load_file(
        ...
        if source is None:
            ...
            source, mtime = await Function.hass.async_add_executor_job(
                read_file, file_path
            )
